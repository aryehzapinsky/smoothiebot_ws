...
if __name__ =="__main__":
	...
	graspy = manager.construct_graspit_grasp(detected_blocks.values()[2].position, detected_blocks.values()[2].orientation)
	manager.execute_grasp("apple", graspy)	
#	print(detected_blocks)
#	print(detected_blocks.values()[0])
#	print(type(detected_blocks.values()[0]))
	#plan grasp reachability
	#apple = detected_blocks.items()[0]
        #grasp = manager.plan_grasps(apple[1].position.x, apple[1].position.y, apple[1].position.z)
        #manager.analyze_grasp_reachability(apple[0], grasp[0])
	#print(manager.grasping_controller.arm_move_group.get_planning_frame())
	for block, val in detected_blocks.items():
		print("BLOCK", block)
		if block == "apple":
			grasps = manager.plan_grasps()
			print("GRASPS", grasps)
			manager.analyze_grasp_reachability(block, grasps[0])
#			manager.execute_grasps(block, grasps[0])
